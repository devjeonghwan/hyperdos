...
{
    hyperdos_pc_bios_data_area_write_byte(pc, offset, (uint8_t)(value & HYPERDOS_X86_16_LOW_BYTE_MASK));
...
{
    hyperdos_pc_bios_data_area_write_word(pc, offset, (uint16_t)(value & HYPERDOS_X86_16_WORD_MASK));
    hyperdos_pc_bios_data_area_write_word(pc,
                                          (uint16_t)(offset + HYPERDOS_X86_16_WORD_SIZE),
...
uint32_t hyperdos_pc_bios_data_area_read_double_word(hyperdos_pc* pc, uint16_t offset)
{
    ...
    uint32_t highWord = hyperdos_pc_bios_data_area_read_word(pc, (uint16_t)(offset + HYPERDOS_X86_16_WORD_SIZE));
...
{
    return (uint16_t)(HYPERDOS_PC_BIOS_DATA_AREA_CURSOR_POSITION_OFFSET + videoPage * HYPERDOS_X86_16_WORD_SIZE);
}
